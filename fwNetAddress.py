...
class fwNetAddress:
    ...
    def _is_valid_fqdn(self, fqdn):
        ...
        # Проверяем наличие хотя бы одной точки в строке
        if '.' not in fqdn:
            return False
        ...
        # Проверяем, что нет пустых частей (случаи с '..' или точкой в начале)
        if '' in parts:
            return False

        # Проверяем, что каждая часть соответствует требованиям
