...
def create_fortigate_policy(firewall, policy_data):
    try:
        ...
        policy_json = {
            ...
            "service": [{"name": svc} for svc in policy_data['svc']],
            "action": "accept",
            "status": "disable",
            "comments": "Создано автоматически",
            "schedule": "always"
        }

        if policy_data['after']:
            policy_json['policyid'] = policy_data['after'] + 1
    ...
    try:
        ...
        for fw, policies in myCode.policies.items():
            ...
            for policy in policies:
                #success = create_fortigate_policy(firewall, policy)
                success = False
